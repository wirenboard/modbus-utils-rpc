...
def request_handle(args):
    ...
    if func_type == 0x01:
        ...
        msg_byte = lib.read_discrete_inputs(
            args.slaveAddr, args.startAddr, args.readNo)
    elif func_type == 0x03:
        ...
        msg_byte = lib.write_single_coil(
            args.slaveAddr, args.startAddr, hex_data_list[0])
    elif func_type == 0x06:
        msg_byte = lib.write_single_register(
            args.slaveAddr, args.startAddr, hex_data_list[0])
    elif func_type == 0x0F:
        msg_byte = lib.write_multiple_coils(
            args.slaveAddr, args.startAddr, hex_data_list)
    elif func_type == 0x10:
    ...
    mqtt_client = mosquitto.Mosquitto("modbus-utils-rpc-%d" % os.getpid())
    ...
    resp = rpc_client.call('wb-mqtt-serial', 'port',
                           'Load', rpc_parameters, args.timeout)
    ...
    data = lib.parse_response_adu(
        bytearray.fromhex(resp['response']), msg_byte)
